...
kern_return_t
get_registers_arm64(mach_port_name_t task, arm_thread_state64_t *state) {
	kern_return_t kret;
...
single_step(thread_act_t thread) {
	...
	kret = thread_get_state(thread, ARM_THREAD_STATE64, (thread_state_t)&regs, &count);
	if (kret != KERN_SUCCESS) return kret;
	...
	kret = thread_set_state(thread, ARM_THREAD_STATE64, (thread_state_t)&regs, count);
	if (kret != KERN_SUCCESS) return kret;
...
clear_trap_flag(thread_act_t thread) {
	...
	// TODO: find out how to work with flags
	//regs.__rflags ^= 0x100UL;

	return thread_set_state(thread, ARM_THREAD_STATE64, (thread_state_t)&regs, count);
}
#else
kern_return_t
clear_trap_flag(thread_act_t thread) {
